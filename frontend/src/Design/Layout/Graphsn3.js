...
const Graphsn3 = (prop) => {
  ...
  const fetchData = async () => {
    try {
      // setLink(prop.link);
      const response = await fetch(`${config.backendUrl}/history4`);
      if (!response.ok) {
      ...
      let last200Elements = result;
      //   console.log("Fetched data:", result);

      result = last200Elements.slice(-600);
  ...
  const processChartData = (data) => {
    ...
    const valuesAngleX = data.map((entry) =>
      Math.abs(entry.data.device1.angleX)
    );
    const valuesAngleY = data.map((entry) =>
      Math.abs(entry.data.device1.angleY)
    );
    const valuesAngleZ = data.map((entry) =>
      Math.abs(entry.data.device1.angleZ)
    );
  ...
  return (
    <div>
      <h4>Device Angles (X, Y, Z)</h4>
      <Line data={chartDataAngles} />
      <h4 className="mt-4">Door Temperature</h4>
      ...
      */}
      ...
      <h4 className="mt-4">Door State | Grid State | Battery Voltage</h4>
      <Line data={chartDataDoor} options={options} />
    </div>
  );
};

export default Graphsn3;
